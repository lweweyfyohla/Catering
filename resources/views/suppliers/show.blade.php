<x-layouts.app title="{{ $supplier->name }} - CaterSource" page-title="Suppliers"
    :page-subtitle="now()->format('l, F j, Y')">

    <div x-data="{ selectedEventId: {{ $selectedEvent?->id ?? 'null' }} }">

        <a href="{{ route('suppliers.index') }}"
            class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-brand-600 mb-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-6">
            ...
            <div class="px-6 py-5 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div
                        class="h-9 w-9 -mt-4 rounded-lg ring-4 ring-white bg-brand-50 shadow-sm overflow-hidden flex items-center justify-center shrink-0">
                        @if ($supplier->logo)
                            <img src="{{ asset('storage/' . $supplier->logo) }}" class="h-full w-full object-cover"
                                alt="{{ $supplier->name }} logo">
                        @else
                            <span
                                class="text-lg font-bold text-brand-500">{{ strtoupper(substr($supplier->name, 0, 2)) }}</span>
                        @endif
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">{{ $supplier->name }}</h2>
                        <p class="text-sm text-slate-400">{{ ucfirst($supplier->category) }} &middot;
                            {{ $supplier->address ?? 'No address' }} &middot; ★ {{ number_format($supplier->stars, 1) }}
                        </p>
                    </div>
                </div>
        ...
        @elseif (!$selectedEvent)
            <div class="mb-6 rounded-lg bg-amber-50 border border-amber-100 text-amber-700 text-sm px-4 py-3">
                ...
                <button onclick="window.dispatchEvent(new CustomEvent('open-modal', {detail: 'select-event'}))"
                    class="font-medium underline">Select event</button>.
            </div>
        @endif

        @if ($supplier->menuItems->isEmpty())
            <div
                class="bg-white rounded-2xl border border-slate-100 shadow-sm px-6 py-12 text-center text-sm text-slate-400">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                @foreach ($supplier->menuItems as $item)
                    <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
                        ...
                        <div class="bg-slate-50 flex items-center justify-center shrink-0"
                        ...
                        <div class="p-3 flex flex-col flex-1">
                            <p class="text-xs text-slate-400 leading-snug min-h-[2rem] line-clamp-2">
                                {{ $item->description }}</p>
                            <div class="mt-auto pt-2">
                                <h4 class="font-semibold text-slate-900 text-sm leading-tight truncate">{{ $item->item_name }}</h4>
                                <p class="text-brand-600 font-bold text-xs leading-tight mt-1">${{ number_format($item->price, 2) }}
                                </p>
                            </div>
                            <button @if($selectedEvent)
                                onclick="window.dispatchEvent(new CustomEvent('open-modal', {detail: 'item-{{ $item->id }}'}))"
                            @else disabled @endif
                    ...
                    <x-modal :name="'item-' . $item->id" max-width="sm">
                        <form method="POST" action="{{ route('cart.store', $item) }}">
                            ...
                            <div class="aspect-square bg-slate-50 flex items-center justify-center" style="aspect-ratio: 1 / 1; width: 100%;">
                            @if ($item->image)
                                <img src="{{ asset('storage/'.$item->image) }}" class="h-full w-full object-cover" style="width: 100%; height: 100%; object-fit: cover; display: block;" alt="">
                                @else
                            ...
                            <div class="px-6 py-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500">
                                        ...
                                        <button type="button"
                                            onclick="window.dispatchEvent(new CustomEvent('open-modal', {detail: 'select-event'}))"
                                            class="text-brand-600 underline text-xs ml-1">change</button>
                                    </p>
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Note</label>
                                    <input type="text" name="note" placeholder="Add your request ..."
                            ...
                            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100">
                                <button type="submit"
                                    class="w-full rounded-lg bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium py-2.5">Add
                                    to Cart</button>
                            </div>
                        </form>
                    </x-modal>
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <select x-model="selectedEventId" class="w-full rounded-lg border-slate-200 text-sm">
                    @foreach ($events as $ev)
                        <option value="{{ $ev->id }}" @selected($selectedEvent && $selectedEvent->id === $ev->id)>

                    ...
                    <tbody class="divide-y divide-slate-50">
                        @foreach ($events as $event)
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="status-pill bg-slate-100 text-slate-500">{{ ucfirst($event->event_type) }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <button
                                            onclick="window.dispatchEvent(new CustomEvent('open-modal', {detail: 'event-{{ $event->id }}'}))"
                                        ...
                                        @else
                                            <a href="{{ route('suppliers.index', ['event_id' => $event->id]) }}"
                                                class="inline-flex items-center gap-1 rounded-lg bg-slate-50 text-slate-500 text-xs font-medium px-3 py-1.5 hover:bg-slate-100">
                                                Browse suppliers
                                            </a>
                                        @endif
                                    </div>
    ...
    <x-modal name="new-event" max-width="lg">
        <form method="POST" action="{{ route('events.store') }}">
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100">
                ...
                <button type="submit"
                    class="rounded-lg bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium px-4 py-2">Create
                    event</button>
            </div>
        </form>
    </x-modal>
            ...
            <div x-data="{ editing: false }">
                ...
                <div x-show="!editing" class="px-6 py-5">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-slate-800 mt-1">{{ $event->event_date->format('F j, Y') }}
                            </p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-slate-800 mt-1">{{ $event->created_at->format('M j, Y') }}
                            </p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-400">Latest updated</p>
                            <p class="text-sm font-medium text-slate-800 mt-1">{{ $event->updated_at->format('M j, Y') }}
                            </p>
                        </div>
                    </div>
                    <div class="mt-4">
                        <p class="text-xs text-slate-400 mb-1">Notes</p>
                        <p class="text-sm text-slate-600 bg-slate-50 rounded-lg px-3 py-2.5">
                            {{ $event->notes ?: 'No additional notes provided.' }}</p>
                    </div>
                </div>

                {{-- Edit mode --}}
                <form x-show="editing" x-cloak method="POST" action="{{ route('events.update', $event) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="guest_count" value="{{ $event->guest_count }}" min="1" required
                            class="w-full rounded-lg border-slate-200 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Notes</label>
                        <textarea name="notes" rows="3"
                            class="w-full rounded-lg border-slate-200 text-sm">{{ $event->notes }}</textarea>
                    </div>
                    <div class="flex justify-end gap-3 pt-2">
                        <button type="button" @click="editing = false"
                            class="rounded-lg border border-slate-200 text-slate-600 text-sm font-medium px-4 py-2">Cancel</button>
                        <button type="submit"
                            class="rounded-lg bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium px-4 py-2">Save
                            changes</button>
                    </div>
                </form>
                ...
                <div x-show="!editing" class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100">
                    <form method="POST" action="{{ route('events.destroy', $event) }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="rounded-lg border border-red-200 text-red-600 text-sm font-medium px-4 py-2 hover:bg-red-50">
                            Delete
                        </button>
                    </form>
                    ...
                    <button type="button" @click="editing = true"
                        class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 text-slate-600 text-sm font-medium px-4 py-2 hover:bg-slate-50">
                        Edit
                    </button>
                    <a href="{{ route('quotations.compare', $event) }}"
                        class="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium px-4 py-2">
                        Request Quote
                    </a>
                </div>
            </div>

<x-layouts.supplier title="Quotation Requests" page-title="Quotation Requests"
                ...
                @if($quotation->status === 'pending')
                    ...
                    <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100">
                        ...
                        <form method="POST" action="{{ route('supplier.quotations.update-status', $quotation) }}"
                            onsubmit="this.querySelector('button').disabled = true; this.querySelector('button').textContent = 'Rejecting...';">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="status" value="rejected">
                            <button class="px-5 py-2 rounded-lg border border-red-300 text-red-600 hover:bg-red-50">
                                Reject
                            </button>
                        </form>

                        <form method="POST" action="{{ route('supplier.quotations.update-status', $quotation) }}"
                            onsubmit="this.querySelector('button').disabled = true; this.querySelector('button').textContent = 'Accepting...';">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="status" value="accepted">
                            <button class="px-5 py-2 rounded-lg bg-brand-500 text-white hover:bg-brand-600">
                                Accept Quote
                            </button>
                        </form>

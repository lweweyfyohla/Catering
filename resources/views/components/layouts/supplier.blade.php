...
<body class="min-h-screen bg-[#f6f7fb] font-sans antialiased text-slate-800" x-data="{ sidebarOpen: false }">
                                ...
                                @foreach ($items as $item)
                                    <a href="{{ route($item['route']) }}"
                                       class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition
                                              {{ request()->routeIs($item['route'].'*') ? 'bg-brand-50 text-brand-600' : 'text-slate-600 hover:bg-slate-50' }}">
    ...
    @endif
                    ...
                    <div class="min-w-0">
                        <h3 class="font-semibold text-slate-900 truncate">{{ auth()->user()->name }}</h3>
                        <p class="text-xs text-slate-400 truncate">{{ auth()->user()->supplier?->name }}</p>
                    </div>
                </div>
        ...
        <main class="flex-1 min-w-0 lg:ml-64">
            <header class="hidden lg:flex items-center justify-between px-8 py-5 border-b border-slate-100 bg-white">
                <div>
                    <h1 class="text-lg font-semibold text-slate-900">{{ $pageTitle ?? 'Supplier Portal' }}</h1>

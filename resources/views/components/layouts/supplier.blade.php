...
<body class="min-h-screen bg-[#f6f7fb] font-sans antialiased text-slate-800">
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-8 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden sm:block">
                    ...
                    <p class="text-xs text-slate-400">{{ auth()->user()->supplier?->name }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('supplier.logout') }}">
                    @csrf
                    <button type="submit" class="text-xs font-medium text-slate-400 hover:text-brand-600">Log out</button>
                </form>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-8 py-8">
        <div class="mb-6">
            <h1 class="text-lg font-semibold text-slate-900">{{ $pageTitle ?? 'Quote Requests' }}</h1>
